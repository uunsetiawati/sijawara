...
class LegalitasUsahaController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $legalitasUsaha = LegalitasUsaha::where(function($q) use ($request) {
                $q->where('name', 'LIKE', '%'.$request->search.'%');
            })->orderBy('id','asc')->paginate($per, ['*', DB::raw('@angka  := @angka  + 1 AS angka')]);

            $legalitasUsaha->map(function($a) {
            ...
            return response()->json($legalitasUsaha);
    ...
    {
    	$legalitasUsaha = LegalitasUsaha::orderBy('id','asc')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    	return RestApi::success($legalitasUsaha);
    }
    ...
    public function create(Request $request)
    {
        if (request()->wantsJson() && request()->ajax()) {
            ...
            $legalitasUsaha = LegalitasUsaha::create($request->all());
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $legalitasUsaha = LegalitasUsaha::findByUuid($uuid);

            if (!isset($legalitasUsaha->id)) {
                return RestApi::error('Legalitas Usaha tidak ditemukan.', 404);
            }
    ...
    public function update(Request $request, $uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            if ($legalitasUsaha->update($request->all())) {
