...
class MasalahUkmController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $masalahUkm = MasalahUkm::where(function($q) use ($request) {
                $q->where('name', 'LIKE', '%'.$request->search.'%');
            })->orderBy('id','asc')->paginate($per, ['*', DB::raw('@angka  := @angka  + 1 AS angka')]);

            $masalahUkm->map(function($a) {
            ...
            return response()->json($masalahUkm);
    ...
    {
    	$masalahUkm = MasalahUkm::orderBy('id','asc')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    	return RestApi::success($masalahUkm);
    }
    ...
    public function create(Request $request)
    {
        if (request()->wantsJson() && request()->ajax()) {
            ...
            $masalahUkm = MasalahUkm::create($request->all());
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $masalahUkm = MasalahUkm::findByUuid($uuid);

            if (!isset($masalahUkm->id)) {
                return RestApi::error('Masalah UKM tidak ditemukan.', 404);
            }
    ...
    public function update(Request $request, $uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            if ($masalahUkm->update($request->all())) {
