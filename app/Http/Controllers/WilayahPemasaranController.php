...
class WilayahPemasaranController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $wialayahPemasaran = WilayahPemasaran::where(function($q) use ($request) {
                $q->where('name', 'LIKE', '%'.$request->search.'%');
            })->orderBy('id','asc')->paginate($per, ['*', DB::raw('@angka  := @angka  + 1 AS angka')]);

            $wialayahPemasaran->map(function($a) {
            ...
            return response()->json($wialayahPemasaran);
    ...
    {
    	$wialayahPemasaran = WilayahPemasaran::orderBy('id','asc')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    	return RestApi::success($wialayahPemasaran);
    }
    ...
    public function create(Request $request)
    {
        if (request()->wantsJson() && request()->ajax()) {
            ...
            $wialayahPemasaran = WilayahPemasaran::create($request->all());

            if($wialayahPemasaran) {
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $wialayahPemasaran = WilayahPemasaran::findByUuid($uuid);

            if (!isset($wialayahPemasaran->id)) {
                return RestApi::error('Wilayah Pemasaran tidak ditemukan.', 404);
            }
            
            return response()->json([
                'status' => true,
                'data' => $wialayahPemasaran
            ]);
        }else{
            return RestApi::error('404 Not Found.', 404);
        }
    ...
    public function update(Request $request, $uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            if ($wialayahPemasaran->update($request->all())) {
