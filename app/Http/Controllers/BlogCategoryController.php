...
class BlogCategoryController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $category = BlogCategory::where(function($q) use ($request) {
    ...
    public function show()
    {
    	...
    	$category = BlogCategory::where($lang)->orderBy('nm_category', 'ASC')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $category = BlogCategory::findByUuid($uuid);
