...
class BentukKoperasiController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $bentukKoperasi = BentukKoperasi::where(function($q) use ($request) {
                $q->where('name', 'LIKE', '%'.$request->search.'%');
            })->orderBy('id','asc')->paginate($per, ['*', DB::raw('@angka  := @angka  + 1 AS angka')]);

            $bentukKoperasi->map(function($a) {
            ...
            return response()->json($bentukKoperasi);
    ...
    {
    	$bentukKoperasi = BentukKoperasi::orderBy('name', 'ASC')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    	return RestApi::success($bentukKoperasi);
    }
    ...
    public function create(Request $request)
    {
        if (request()->wantsJson() && request()->ajax()) {
            ...
            $bentukKoperasi = BentukKoperasi::create($request->all());
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $bentukKoperasi = BentukKoperasi::findByUuid($uuid);

            if (!isset($bentukKoperasi->id)) {
                return RestApi::error('Bentuk Koperasi tidak ditemukan.', 404);
            }
    ...
    public function update(Request $request, $uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            if ($bentukKoperasi->update($request->all())) {
