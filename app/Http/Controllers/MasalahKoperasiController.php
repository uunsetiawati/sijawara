...
class MasalahKoperasiController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $masalahKoperasi = MasalahKoperasi::where(function($q) use ($request) {
                $q->where('name', 'LIKE', '%'.$request->search.'%');
            })->orderBy('id','asc')->paginate($per, ['*', DB::raw('@angka  := @angka  + 1 AS angka')]);

            $masalahKoperasi->map(function($a) {
            ...
            return response()->json($masalahKoperasi);
    ...
    {
    	$masalahKoperasi = MasalahKoperasi::orderBy('name', 'ASC')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    	return RestApi::success($masalahKoperasi);
    }
    ...
    public function create(Request $request)
    {
        if (request()->wantsJson() && request()->ajax()) {
            ...
            $masalahKoperasi = MasalahKoperasi::create($request->all());
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $masalahKoperasi = MasalahKoperasi::findByUuid($uuid);

            if (!isset($masalahKoperasi->id)) {
                return RestApi::error('Masalah Koperasi tidak ditemukan.', 404);
            }
            
            return response()->json([
                'status' => true,
                'data' => $masalahKoperasi
            ]);
        }else{
            return RestApi::error('404 Not Found.', 404);
        }
    ...
    public function update(Request $request, $uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            if ($masalahKoperasi->update($request->all())) {
