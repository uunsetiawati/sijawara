...
class SektorUsahaController extends Controller
{
    public function index(Request $request)
    {
    	if (request()->wantsJson() && request()->ajax()) {
            ...
            $sektorUsaha = SektorUsaha::where(function($q) use ($request) {
                $q->where('name', 'LIKE', '%'.$request->search.'%');
            })->orderBy('id','asc')->paginate($per, ['*', DB::raw('@angka  := @angka  + 1 AS angka')]);

            $sektorUsaha->map(function($a) {
            ...
            return response()->json($sektorUsaha);
    ...
    {
    	$sektorUsaha = SektorUsaha::orderBy('name', 'ASC')->get()->makeHidden(['created_at', 'updated_at', 'uuid']);;
    	return RestApi::success($sektorUsaha);
    }
    ...
    public function create(Request $request)
    {
        if (request()->wantsJson() && request()->ajax()) {
            ...
            $sektorUsaha = SektorUsaha::create($request->all());
    ...
    public function edit($uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            $sektorUsaha = SektorUsaha::findByUuid($uuid);

            if (!isset($sektorUsaha->id)) {
                return RestApi::error('Sektor Usaha tidak ditemukan.', 404);
            }
    ...
    public function update(Request $request, $uuid)
    {
        if (request()->wantsJson() && request()->ajax() && strlen($uuid) == 36) {
            ...
            if ($sektorUsaha->update($request->all())) {
