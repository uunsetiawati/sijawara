        ...
        <transition enter-active-class="animated slideInDown">
            <div v-if="showIndex" class="card card-custom">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <mti-paginate url="/course/index" id="tableIndex" ref="tableIndex" classx="rounded table table-hover table-secondary" :columns="courseColumns" classHead="pt-2" :callback="loadTable"></mti-paginate>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-12 form-group">
                                ...
                                <input type="text" class="form-control" name="nm_course" placeholder="Nama Kursus" v-model="formRequest.nm_course" required/>
                            </div>
                            <div class="col-md-12 form-group">
                                ...
                                <div class="row form-inline">
                                    ...
                                    <div class="col-1">
                                       <span class="switch switch-sm switch-success">
                                            <label>
                                                <input type="checkbox" :checked="((type == 'create') ? 'checked' : ((formRequest.status == '1') ? 'checked' : false))" name="status"/>
                                                <span></span>
                                            </label>
                                        </span>
                                    </div>
                            ...
                            <div class="col-md-6 form-group">
                                <label>Image <span v-if="type != 'create'" style="color: red;font-size: 10px;" class="">* jangan ubah jika tidak ada berubahan!</span></label>
                                <div class="row form-inline">
                                    <label class="col-3 col-form-label">Image Link</label>
                                    <div class="col-1">
                                       <span class="switch switch-sm switch-primary">
                                            <label>
                                                <input @change="changeSwitchImage" type="checkbox" v-model="imageChecked"/>
                                                <span></span>
                                            </label>
                                        </span>
                                    </div>
                                ...
                                <input v-if="!imageChecked" class="form-control" type="text" name="image" placeholder="Image Link">
                                <div v-else class="h-225px image-input image-input-empty image-input-outline w-375px" id="imageUpload" :style="((type == 'create') ? 'background-image: url(assets/media/placeholder/375x225.png)' : `background-image: url(${formRequest.image_url})`)">
                            ...
                            <div class="col-md-6 form-group">
                                <label>Module <span v-if="type != 'create'" style="color: red;font-size: 10px;" class="">* jangan ubah jika tidak ada berubahan!</span></label>
                                <div class="row form-inline">
                                ...
                                <input v-if="!moduleChecked" class="form-control" v-model="formRequest.module" type="text" name="module" placeholder="Google Drive Link">
        ...
        data() {
            return {
                courseColumns: [
                    ...
                    {name: '#', data: 'image'},
                    {name: 'Nama Kursus', data: 'nm_course', style: 'width: 235px;'},
                    {name: 'Overview', data: 'overview'},
                    {name: 'Status', data: 'status'},
                    {name: 'Bahasa', data: 'lang'},
                    {name: 'Aksi', data: 'action', style: 'width: 370px;'},
                ],
        ...
        methods: {
            loadTable() {
                ...
                $('#tableIndex').on('click', '.peserta', function(e) {
                    ...
                    KTApp.block(el)
                    var id = $(this).data('id');
                    vm.$router.push({name: 'admin.course.list.peserta', params: {uuid: id}});
                });

                $('#tableIndex').on('click', '.syllabus', function(e) {
                    vm.$router.push({name: 'admin.course.list.content', params: {uuid: $(this).data('id')}});
                });
            ...
            sendData() {
                ...
                formData.append('overview', $('.summernote').summernote('code'));
                formData.append('categories', vm.formRequest.categories.join(','));
                var url = '/course/create';
                if(vm.type != 'create') {
                    url = `/course/${vm.formRequest.uuid}/update`;
                    formData.append('user_id', vm.formRequest.user_id);
                }
                vm.$http({
                    url: url,
                    method: 'POST',
            ...
            loadNotes(withContent=false) {
                ...
                if(withContent){
                    $(".summernote").summernote("code", this.formRequest.overview);
                }
            },
            changeSwitchImage() {
                var vm = this;
