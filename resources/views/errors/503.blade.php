...
<html lang="en">
	...
	<head><base href="/">
		<meta charset="utf-8" />
		<title>{{ explode('|',json_decode(file_get_contents(storage_path('framework/down')), true)['message'])[0] }}</title>
		...
		<link href="assets/css/pages/error/error-6.css?v=7.0.5" rel="stylesheet" type="text/css" />
		...
		<div class="d-flex flex-column flex-root">
			...
			<div class="error error-6 d-flex flex-row-fluid bgi-size-cover bgi-position-center" style="background-image: url(assets/media/error/bg6.jpg);">
				...
				<div class="d-flex flex-column flex-row-fluid text-center">
					<h1 class="error-title font-weight-boldest text-white mb-12" style="margin-top: 7rem;">{{ explode('|',json_decode(file_get_contents(storage_path('framework/down')), true)['message'])[0] }}</h1>
					<p class="display-4 font-weight-bold text-white">{!! explode('|',json_decode(file_get_contents(storage_path('framework/down')), true)['message'])[1] !!}</p>
				</div>
				<!--end::Content-->
			</div>
			<!--end::Error-->
